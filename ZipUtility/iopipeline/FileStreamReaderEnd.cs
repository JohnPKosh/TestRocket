...
namespace iopipeline
{
  ...
  public class FileStreamReaderEnd
  {
    ...
    public async Task<string> PerformRead()
    {
      try
      {
        ...
        sw.Start();
        //var rv = await RunStreamReaderAsync().ConfigureAwait(false);
        var rv = RunStreamReader();
        sw.Stop();
    ...
    private async Task<string> RunStreamReaderAsync()
    {
    ...
    private string RunStreamReader()
    {
      var rv = string.Empty;
      using (var sr = new StreamReader(m_Stream))
      {
